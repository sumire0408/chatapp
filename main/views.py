...
def signup(request):
    ...
    if request.method == "GET":
        form = SignUpForm()
        # print(form)
    elif request.method == "POST":   # POSTはパスワードなどの見られてはいけないものを送信する時に使う
        ...
        if form.is_valid():
            ...
            return redirect("index")   # 目次ページにリダイレクトする

    context = {"form": form}
    return render(request, "main/signup.html", context)   # 正しい場合、elifを無視してここへ
